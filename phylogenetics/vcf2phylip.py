    ...
    with opener(vcf_file, "rt") as vcf:
        for line in vcf:
            ...
            if line.startswith("#CHROM"):
                record = line.split("\t")
                sample_names = [record[i].replace(
                    "./", "") for i in range(9, len(record))]
                break
    return sample_names
...
def main():
    ...
    with opener(args.filename, "rt") as vcf:
                            ...
                            if is_snp(record):
                                ...
                                if args.nexusbin:
                                    ...
                                    if len(record[4]) == 1:
                                        ...
                                        binsite_tmp = get_matrix_column_bin(
                                            record, num_samples)
        ...
        if args.nexusbin:
            print("Biallelic SNPs selected for binary NEXUS: {:d}".format(
                snp_biallelic))
    ...
    if outgroup in sample_names:
        ...
        if args.fasta or args.nexus or not args.phylipdisable:
            with open(outfile+".tmp") as tmp_seq:
                ...
                if args.fasta:
                    output_fas.write(
                        ">"+sample_names[idx_outgroup]+"\n"+seqout+"\n")

                # Pad sequences names and write PHYLIP or NEXUS lines
                padding = (len_longest_name + 3 -
                           len(sample_names[idx_outgroup])) * " "
                if not args.phylipdisable:
                    output_phy.write(
                        sample_names[idx_outgroup]+padding+seqout+"\n")
                if args.nexus:
                    output_nex.write(
                        sample_names[idx_outgroup]+padding+seqout+"\n")
